...
open class SwiftAddressBookPerson : SwiftAddressBookRecord {
	...
	open class func createInSourceWithVCard(_ source : SwiftAddressBookSource, vCard : String) -> [SwiftAddressBookPerson]? {
		let data : Data? = vCard.data(using: String.Encoding.utf8, allowLossyConversion: true)
		let abPersons : NSArray? = ABPersonCreatePeopleInSourceWithVCardRepresentation(source.internalRecord, data as CFData!).takeRetainedValue()
	...
	open class func createVCard(_ people : [SwiftAddressBookPerson]) -> String {
		let peopleArray = people.map { $0.internalRecord }
		let data : Data = ABPersonCreateVCardRepresentationWithPeople(peopleArray).takeRetainedValue() as Data
